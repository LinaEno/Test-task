...
((self) => {
      ...
      if (serializedType === 5) {
        ...
        return new Proxy(new Event(obj.type, obj), {
          get: (target, propName) =>
            propName in obj
              ? obj[propName]
              typeof target[String(propName)] === "function" ?
              ? noop
  ...
  const warnCrossOrgin = (apiType, apiName, env) =>
    console.warn(
      `Partytown unable to ${apiType} cross-origin ${apiName}: ${env.$location$}`
    );
  ...
  const getTargetProp = (target, applyPath) => {
    ...
    if (target) {
      ...
      if (cstrName === "Window") {
        ...
        n =
          "nodeType" in target && target.nodeType === 2 ?
            "attributes."
            ...
            ? "style."
            cstrName === "MutationObserver" ?
            ? "mutationObserver."
            : "NamedNodeMap" === cstrName
            ? "namedNodeMap."
            cstrName === "ResizeObserver" ?
            ? "resizeObserver."
  ...
  const getLogValue = (applyPath, v) => {
    ...
    return ((v) => typeof v === "object" && v && v.then)(v)
      ? "Promise"
      : "function" === type
      ? `ƒ() ${v.name || ""}`.trim()
      : `¯\\_(ツ)_/¯ ${String(v)}`.trim();
    ...
    if (instance[ApplyPathKey]) {
      ...
      taskQueue[len(taskQueue) - 1].$debug$ = ((
        ...
        callType === 1 ?
          ...
          : 2 === callType
          ? (m += " (non-blocking)")
          : callType === 3 && (m += " (non-blocking, no-side-effect)");
        return m.trim();
  ...
  const addStorageApi = (win, storageName, storages, isSameOrigin, env) => {
    ...
    win[storageName] = new Proxy(storage, {
      ...
      has: (target, key) =>
        !!Reflect.has(target, key) ||
        (typeof key === "string" && target.getItem(key) !== null),
  ...
  const run = (env, scriptContent, scriptUrl) => {
    ...
    scriptContent = `with(this){${scriptContent
            ...
            $window$: new Proxy(win, {
              get: (win, propName) => {
                ...
                if (typeof propName !== "string" || isNaN(propName)) {
                  return (
                    (_a = webWorkerCtx.$config$.mainWindowAccessors) === null ||
                    void 0 === _a
                      ? void 0
      ...
      if (msgType === 7) {
        ...
        msgType === 15 &&
          ((_type, winId, instanceId, callbackName, args) => {
            ...
            elm &&
              typeof elm[callbackName] === "function" &&
